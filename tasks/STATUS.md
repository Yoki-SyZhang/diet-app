...
| 1.7  | 食物营养查询:LLM 直接估算(临时,接口预留后续替换四级查询链) | LLM 按食物名+生熟状态直接产出 per-100g 五项营养值+置信度(SPEC §7.2 里程碑说明),`source_tag=llm_estimate`,走 §7.4"LLM 直接估算"确认行(Confirm 后写入,标注"可能不准",无候选消歧) | 契约测试:合法结构率、Confirm 流程正确、`source_tag`/置信度标注正确、缺失营养素为 null 不为 0 | —(待实现,NN=03) | `feature_demo_03_query_engine`(同 PR) | [ ]  |
